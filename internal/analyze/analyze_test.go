...
// analyze_test.go — контрактные тесты для internal/analyze.
//
// Контракт генератора:
...
func (s *Service) DoWork(ctx context.Context) error {
	...
	got := analyze.CollectRepoCalls(src)
...
func (s *Service) DoWork(ctx context.Context) error {
	...
	got := analyze.CollectRepoCallsWithAllowed(src, []string{"*.storage.*"})
	for _, want := range []string{"DirectCall", "AliasCall"} {
		if _, ok := got[want]; !ok {
			t.Errorf("missing repo method %q in %v", want, got)
		}
...
func (s *Service) Load(ctx context.Context) {
	...
	got := analyze.CollectRepoCallsWithAllowed(src, []string{"*.repo.*"})
...
func (h *Handler) A() {
	...
	got := analyze.CollectServiceCalls(src)
...
func (h *Handler) A() {
	...
	got := analyze.CollectServiceCallsWithAllowed(src, []string{"*.svc.*"})
	for _, want := range []string{"DirectSvc", "AliasSvc"} {
		if _, ok := got[want]; !ok {
			t.Errorf("missing service method %q in %v", want, got)
		}
...
func (h *Handler) A() {
	...
	got := analyze.CollectServiceCallsWithAllowed(src, []string{"*.svc.DirectSvc"})
...
`
	...
	got, ok := analyze.InferPortPrefix(pkg)
	if !ok || got != "WebPush" {
...
`
	...
	if _, ok := analyze.InferPortPrefix(pkg); ok {
...
func TestDetectModulePath_readsGoMod(t *testing.T) {
	...
	got, err := analyze.DetectModulePath(dir)
	if err != nil {
...
func TestDetectModulePath_errorWhenNoGoMod(t *testing.T) {
	dir := t.TempDir()
	_, err := analyze.DetectModulePath(dir)
	if err == nil {
...
func TestLoadSources_includesNonTestGoFiles(t *testing.T) {
	...
	bodies, err := analyze.LoadSources(dir)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(bodies) != 3 {
...
func TestLoadSources_errorWhenDirMissing(t *testing.T) {
	_, err := analyze.LoadSources("/nonexistent/portsmith/test/dir")
	if err == nil {
...
`,
	...
	pkgs, err := analyze.LoadModulePackages(root)
	if err != nil {
		t.Fatalf("LoadModulePackages: %v", err)
	}
	repo, svc := analyze.CollectCrossModuleCalls(pkgs, "example.com/mod/internal/bots")
...
`,
	...
	_, svc := analyze.CollectCrossModuleCalls(pkgs, "example.com/mod/internal/bots")
