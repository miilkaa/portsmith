// Package initcmd_yamlguard_test contains a reflection-based guard that ensures
// every field of lintconfig.LintConfig appears in the generated portsmith.yaml.
//
...
// TestBuildPortsmithYAML_containsAllLintConfigFields reflects on lintconfig.LintConfig,
...
func TestBuildPortsmithYAML_containsAllLintConfigFields(t *testing.T) {
	...
	output := initcmd.BuildPortsmithYAMLString(answers, "github.com/example/myapp")

	yamlTags := collectYAMLTags(reflect.TypeOf(lintconfig.LintConfig{}))
...
func TestBuildPortsmithYAML_rootStackField(t *testing.T) {
	answers := initcmd.WizardAnswers{Stack: "gin-gorm"}
	output := initcmd.BuildPortsmithYAMLString(answers, "")
